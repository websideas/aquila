...
<div id="comments" class="comments-area">
    ...
    <?php if ( have_comments() ) : ?>
        <h2 class="comments-title post-single-heading">
            <?php
            printf( _nx( 'One comment', '%1$s comments', get_comments_number(), 'comments title', 'adroit' ),
    ...
    $new_fields = array(
        'author' => '<p class="comment_field-column comment-form-author">' .
            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '"  placeholder="'.esc_html__('Name', 'adroit').'"'. $aria_req . $html_req .'/></p>',
        'email'  => '<p class="comment_field-column comment-form-email">' .
            '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" placeholder="'.esc_html__('Email', 'adroit').'"'. $aria_req . $html_req.'/></p></div>',
        'url'    => '<p class="comment-form-url">' .
            '<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" placeholder="'.esc_html__('Website', 'adroit').'" /></p>',
    );

    $comments_args = array(
        'label_submit'      => esc_html__( 'send','adroit' ),
        'fields' => apply_filters( 'comment_form_default_fields', $new_fields ),
        'comment_field' => '<p><textarea id="comment" name="comment" placeholder="'.esc_html__('Your Comment', 'adroit').'"  aria-required="true" rows="6"></textarea></p>',
